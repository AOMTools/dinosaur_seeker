...
class GuiPart:
    ...
    def animated(self, i):
        ...
        self.figure_subplot.set_ylim([0, max(np.concatenate([self.Tydata, self.Fydata]))*1.1])
        ...
        if trighead_time_index.size > 0:
            ...
            for t, c in np.nditer([time_array_window, channel_array_window]):
                if t > tolook_time:
                    ...
                    if (c & 0x2) == 2:
                        if process_type == 1:
                            data_type1.append(counter)
                            time_type1.append(tolook_time)
                            process_type = 2
                        elif process_type == 2:
                            data_type2.append(counter)
...
if __name__ == '__main__':
    ...
    root = Tkinter.Tk(  )
    root.title("TeFiTi Seeker Version 1.04 Branch 1 Pro")

    img = Tkinter.PhotoImage(file='icon_tftf.png')
